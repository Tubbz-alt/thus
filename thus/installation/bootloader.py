...
class Bootloader(object):
    ...
    def check_root_uuid_in_grub(self):
        ...
        if len(self.root_uuid) == 0:
            logging.warning(_("'ruuid' variable is not set. I can't check root "
                              "UUID in grub.cfg, let's hope it's ok"))
            return
        ruuid_str = 'root=UUID=' + self.root_uuid
        boot_command = self.settings.get('GRUB_CMDLINE_LINUX')
        if boot_command is None:
            boot_command = ''
        boot_command = ('linux /{0} {1} {2}\n'
                        .format(self.vmlinuz, ruuid_str, boot_command))
    ...
    def modify_grub_default(self):
        ...
        GRUB_CMDLINE_LINUX_DEFAULT :
            Unless ‘GRUB_DISABLE_RECOVERY’ is set to ‘true’, two menu
            ...
            arguments to add only to the default menu entry, after those listed
        ...
        logging.debug(_("/etc/default/grub configuration completed "
                        "successfully."))
    ...
    def set_grub_option(self, option, cmd):
        ...
        try:
            ...
            logging.debug('Set {0}="{1}" in /etc/default/grub'
                          .format(option, cmd))
    ...
    def random_generator(size=4, chars=string.ascii_lowercase + string.digits):
        """
        Generates a random string to be used as an identifier for
        the UEFI bootloader_id
        """
    ...
    def apply_osprober_patch(self):
        ...
        """
        osp_path = os.path.join(self.dest_dir,
        ...
        if os.path.exists(osp_path):
            ...
            logging.warning(_("Failed to patch 50mounted-tests, "
                              "file not found."))
    ...
    def copy_grub2_theme_files(self):
        ...
        theme_dir_dst = os.path.join(self.dest_dir,
                                     "boot/grub/themes/Manjaro-Default")
        try:
    ...
    def install_grub2_locales(self):
        ...
        grub_mo = os.path.join(self.dest_dir,
    ...
    def freeze_unfreeze_xfs(self):
        ...
        try:
            ...
            # We leave a blank space in the end as we want to search exactly
            # for this mount points
