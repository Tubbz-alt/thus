...
COMMON_MOUNT_POINTS = ['/', '/boot', '/home', '/var']
COMMON_MOUNT_POINTS_EFI = ['/', '/boot', '/boot/efi', '/home', '/var']
...
def get_uuid(part):
    ...
    if "UUID" in info.keys():
        ...
        logging.error(_("Can't get partition {0} UUID").format(part))
        return ""
...
def get_label(part):
    ...
    if "LABEL" in info.keys():
        ...
        logging.debug(_("Can't get partition {0} label (or it does not have any)").format(part))
        return ""
